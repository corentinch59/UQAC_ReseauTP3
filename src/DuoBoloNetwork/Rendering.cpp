...
void Renderer::Render(entt::registry& world, const Camera& camera, const WorldSettings& wSettings)
{
	Vector3 lightDir = Vector3Normalize(wSettings.lightDirection);
	mLightCam.position = camera.position + Vector3Scale(lightDir, -15.0f);
	Vector4 lightColorNormalized = ColorNormalize(wSettings.lightColor);
	Vector4 ambient = ColorNormalize(wSettings.ambientColor);
	UpdateLightCam(lightDir);

	SetShaderUniforms(lightDir, lightColorNormalized, ambient);
...
{
	mLightCam.target = mLightCam.position + lightDir;
	mLightCam.projection = CAMERA_ORTHOGRAPHIC;
	mLightCam.up = {0.0f, 1.0f, 0.0f};
	mLightCam.fovy = SHADOW_DISTANCE;
}

void Renderer::SetShaderUniforms(Vector3 lightDir, Vector4 lightColorNormalized, Vector4 ambientColorNormalized)
{
	SetShaderValue(mShadowShader, mLightDirLoc, &lightDir, SHADER_UNIFORM_VEC3);
	SetShaderValue(mShadowShader, mLightColLoc, &lightColorNormalized, SHADER_UNIFORM_VEC4);
	SetShaderValue(mShadowShader, mAmbientLoc, &ambientColorNormalized, SHADER_UNIFORM_VEC4);
